...
const ActionColumn = ({ row }: { row: Data }) => {
    ...
    const onEdit = () => {
        navigate(`/app/categories/category-edit/${row.id}`)
    }
...
const ImageColumn = ({ row }: { row: Data }) => {
    const avatar = row.category_logo ? (
        <Avatar size={70} src={appConfig.filePrefixGcp + row.category_logo} />
    ) : (
...
const CategoryTable = () => {
    ...
    const { pageIndex, pageSize, sort, query, total } = useAppSelector(
        (state) => state.homesCategoryList.data.tableData
    )
    const loading = useAppSelector(
        (state) => state.homesCategoryList.data.loading
    )

    const data = useAppSelector(
        (state) => state.homesCategoryList.data.orderList
    )
