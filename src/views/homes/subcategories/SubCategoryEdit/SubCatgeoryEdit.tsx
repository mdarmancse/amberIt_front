...
import SubCategoryForm, {
    ...
    OnDeleteCallback,
    SetSubmitting,
} from '@/views/homes/subcategories/SubCategoryForm'

injectReducer('homeSubCategoryEdit', reducer)

const SubCatgeoryEdit = () => {
    const dispatch = useAppDispatch()

    const location = useLocation()
    const navigate = useNavigate()

    const data = useAppSelector((state) => state.homeSubCategoryEdit.data.data)
    const loading = useAppSelector(
        (state) => state.homeSubCategoryEdit.data.loading
    )
    ...
    const popNotification = (keyword: string) => {
        toast.push(
            <Notification
                ...
                duration={2500}
            >
                SubCategory successfully {keyword}
            </Notification>,
            {
