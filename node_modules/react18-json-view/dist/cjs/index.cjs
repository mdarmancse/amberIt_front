...
function CopyButton({ node }) {
    const { customizeCopy } = React.useContext(JsonViewContext);
...
function LargeArrayNode({ originNode, node, depth, index, deleteHandle: _deleteSelf, customOptions, startIndex }) {
    const { enableClipboard, src, onEdit, onChange, forceUpdate, displaySize } = React.useContext(JsonViewContext);
...
function ObjectNode({ node, depth, indexOrName, deleteHandle: _deleteSelf, customOptions }) {
    const { collapsed, enableClipboard, ignoreLargeArray, collapseObjectsAfterLength, editable, onDelete, src, onAdd, onEdit, onChange, forceUpdate, displaySize } = React.useContext(JsonViewContext);
...
function JsonNode({ node, depth, deleteHandle: _deleteHandle, indexOrName, parent, editHandle }) {
    const { collapseStringsAfterLength, enableClipboard, editable, src, onDelete, onChange, customizeNode, matchesURL, urlRegExp } = React.useContext(JsonViewContext);
    let customReturn;
...
function JsonView({ src, collapseStringsAfterLength = 99, collapseStringMode = 'directly', collapseObjectsAfterLength = 99, collapsed, enableClipboard = true, editable = false, onEdit, onDelete, onAdd, onChange, dark = false, theme = 'default', customizeNode, customizeCopy = stringifyForCopying, displaySize, style, className, matchesURL = false, urlRegExp = defaultURLRegExp, ignoreLargeArray = false }) {
    ...
    return (jsxRuntime.jsx(JsonViewContext.Provider, Object.assign({ value: {
            ...
            urlRegExp,
            ignoreLargeArray
        } }, { children: jsxRuntime.jsx("code", Object.assign({ className: 'json-view' + (dark ? ' dark' : '') + (theme && theme !== 'default' ? ' json-view_' + theme : '') + (className ? ' ' + className : ''), style: style }, { children: jsxRuntime.jsx(JsonNode, { node: src, depth: 1 }) })) })));
}
